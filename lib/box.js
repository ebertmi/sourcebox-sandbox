...
var fs = require('fs'); // not fs-extra!!
...
Box.prototype.setDiskspaceLimit = function (size, callback) {
  ...
  return sbfs.setQuota(rootfs, size).nodeify(callback);
};
...
// TODO maybe allow setting the umask per box? right now node's umask is inherited
...
// Overwrites fs.open with box.openFile, then runs fn. Any nodejs file system
// methods using fs.open will now open a fd in the container and not on the
// host. Restores fs.open after fn returns.
function wrapOpen(box, options, fn) {
  if (!options) {
    options = {};
  }
  ...
  fs.open = function (path, flag, mode, callback) {
    // the mode parameter passed by nodejs is ignored since we are passing way
    // more options then just the mode into openFile
    return box.openFile(path, flag, options, callback);
  };
...
Box.prototype.readFile = function (path, options, callback) {
  ...
  return wrapOpen(this, options, function () {
...
Box.prototype.destroy = function (callback) {
  ...
  return Promise.settle([sbfs.destroyQuota(rootfs), stopRetry])
    .bind(this)
