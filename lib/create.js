...
CreationContext.prototype.cleanup = function () {
  ...
  return Promise.resolve()
    ...
    .then(function () {
      ...
      if (toClean.loop) {
        return sbfs.destroyLoopMount(this.path);
      }
    }).then(function () {
      if (toClean.dir) {
  ...
  return fs.ensureDirAsync(this.path)
    ...
    .then(function () {
      if (this.loopsize) {
        ...
        return sbfs.createLoopMount(file, this.path, this.loopsize)
          .bind(this)
          .tap(function () {
            this.toClean.loop = true;
          });
      } else {
        return sbfs.isBtrfs(this.path);
      }
    })
    .then(function () {
      this.progress('Enabling quota');
      return sbfs.enableQuota(this.path);
    });
};

CreationContext.prototype.createContainer = function (workaround) {
  return lxc.getContainerAsync(constants.SOURCE, this.getOptions)
    .bind(this)
...
CreationContext.prototype.setCreateConfig =  function () {
  ...
  return Promise.using(sbfs.withTempFile(), function (file) {
